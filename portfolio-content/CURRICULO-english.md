...
- **24/7 computer vision in production:** Computer Vision pipelines (YOLO at the edge) integrated with LLMs via google-genai, LangGraph, and RAG for compliance monitoring across a fuel station chain — analysis of detected occurrences and insights generated with GenAI (Gemini), a retraining loop fed by user feedback (AWS S3/Lambda/DynamoDB + GCP), with capture via RTSP straight from the client's Intelbras DVR (secure external port configured via NAT, reusing existing hardware) — live pilot monitoring 2 stations, with rollout planned for the chain's ~50 stations.
...
- **R&D platform with Bayesian optimization:** modernized a previously manual R&D process (trial-and-error greenhouse cycles) into an automated workflow assisted by generative AI (Gemini via google-genai) — the "Colibri" agent — plus stability recommendations via BayBE (Merck's framework) — in production for a fertilizer manufacturer.
...
- **Experimenta AI — Soneca (management in production for ~3 months, handling ~R$ 300K/month in sales):** complete snack bar management — counter, table ordering via QR code, self-service kiosk, and cross-platform ESC/POS printing — plus an own delivery ecosystem built with Java 17 + Angular 20 (customer/courier PWAs, real-time tracking GPS → TTL cache → SSE, Google Maps Platform), currently deactivated.
...
- **google-genai SDK / Responses API** — direct LLM integration in production — my main day-to-day approach, alongside LangGraph orchestration
