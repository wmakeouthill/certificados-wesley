...
- **Plataforma B2B de gestão de pessoas (monorepo DDD):** feature de notificações (serviço dedicado consumindo eventos dos bounded contexts) com entrega via Slack e Outlook — integrações implementadas por mim, usadas por todos os funcionários da empresa — e sistema de logs, respeitando database-per-service e comunicação assíncrona por Pub/Sub.
- **Frontend de demo enterprise (Itaú):** interface pixel-perfect a partir de Figma com mapas interativos (React 19 + Leaflet), em dupla — demo navegável entregue em 1 dia e refinada em 2–3 dias para a apresentação comercial.
...
- **Liderança técnica e colaboração:** liderança técnica de frentes nos projetos em equipe (P&D químico e visão computacional); contato constante com os clientes — pontos semanais e alinhamentos pontuais para garantir que a solução atendesse a necessidade real, com feedback positivo pela parceria no desenho de soluções personalizadas; ativo no fluxo de code review/revisão de PRs e no apoio ao programa de PDI da empresa; disciplina com o processo ágil — ritos (daily, planning, refinamento, retro) e gestão de tarefas no Jira sempre em dia.
...
- **Automação fiscal de NF-e (em produção):** captura de notas por e-mail (Microsoft Graph) com extração em camadas — XML determinístico → PDF → OCR/IA como fallback — FastAPI + React, eliminando digitação manual: processa de 20 a 100 NF-e por dia, com milhares já processadas.
- **Sol — Central omnichannel de atendimento TI com IA (em produção):** atendimento por WhatsApp, Teams, plataforma web e e-mail — na casa de 20–50 chamados/dia, com volume de mensagens bem maior nas tratativas e no módulo de grupos; relatórios automatizados, termômetro de satisfação por amostragem de conversas com pareceres por setor, painel completo de gestão de tickets e módulo de CRM integrado para campanhas — FastAPI + Angular 20 + pgvector + Gemini + Evolution/Meta API.
